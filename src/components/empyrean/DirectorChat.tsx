  ...
  const handleConfirm = useCallback(async (action: DirectorProposedAction) => {
    if (onConfirmAction) {
      try {
        ...
        // Dispatcher already toasted; just leave the card visible so user can retry.
      }
    } else {
      toast.info(`Would ${action.type.replace(/_/g, ' ')} — dispatcher not wired yet.`, { duration: 3500 });
      dismissAction(action.id);
    }

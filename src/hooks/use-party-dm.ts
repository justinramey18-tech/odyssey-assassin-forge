...
export function usePartyDm({ partyId, isCreator, memberCount, characterName, characterContext, partyMembers, customGuidesContent, memoryAnchorsContent, partyDragonConfigs, myDragonName, onBurnoutDetected, onBurnoutTickDetected, onBondStrainDetected, onBondGrowthDetected, onDragonMemoryDetected, isSoloEmpyrean }: UsePartyDmOptions) {
  ...
  const startNpcScene = useCallback(async (npcs: string[], scenePrompt: string, maxMessages: number = 12) => {
    ...
    // Atomic database lock — same pattern as voiceNPC
    const { data: lockData, error: stateErr } = await (supabase.from('party_shared_state') as any)
    ...
    try {
      ...
      for (let turn = 0; turn < maxMessages; turn++) {
        ...
        // 1.5s delay between NPC lines (abort-aware)
        if (turn > 0) {
...
- Stay in character as ${currentNpc} has been portrayed.`;
        ...
        // Call ai-dm edge function — same fetch+stream pattern as voiceNPC
        const authToken = await getAuthToken();
        const npcResponse = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/ai-dm`, {
          method: 'POST',
          headers: {
        ...
        // Stream SSE — same reader/decoder/buffer pattern as voiceNPC
        const npcReader = npcResponse.body.getReader();
        const npcDecoder = new TextDecoder();
        let npcBuffer = '';
        let assistantContent = '';
      ...
      npcSceneActiveRef.current = false;
      // Release lock and clear scene state
      await (supabase.from('party_shared_state') as any)

...
export function useAIDM({ characterContext, customGuidesContent, worldStatePrompt, dmPersonaPrompt, responseModePrompt, onMessageComplete, onQuestExtracted, activeGuideIds, onCampaignSwitch, selectedModel, sessionStorageKey, summarizeStorageKey, activeCampaignIdKey, coreRulesInGuides }: UseAIDMOptions) {
  ...
  const voiceNPC = useCallback(async (
    ...
    const isInsightSuccess = socialCheck?.skill === 'insight' && socialCheck?.outcome === 'success';

    const socialCheckBlock = socialCheck
      ? `\n\n## SOCIAL CHECK RESOLVED\nThe player attempted a ${socialCheck.skill} check against ${names[0]}. Player total: ${socialCheck.playerTotal}. ${names[0]}'s opposing ${socialCheck.opposingSkillLabel}: ${socialCheck.npcTotal}. Outcome: ${socialCheck.outcome === 'success' ? 'SUCCESS — the attempt works.' : socialCheck.outcome === 'failure' ? 'FAILURE — the attempt does not work.' : 'TOO CLOSE TO CALL — treat as an ambiguous, partial result.'}\n\nLet the dialogue and physical beat reflect this outcome naturally. Never mention numbers, dice, or game mechanics in the dialogue itself. A SUCCESS on Persuasion or Deception means ${names[0]} is genuinely swayed or fooled, even if reluctant about it. A FAILURE means they see through it, resist, or push back — possibly now more guarded or annoyed. A SUCCESS on Intimidation means real fear or capitulation; a FAILURE means defiance, anger, or calling the bluff.${socialCheck.skill === 'insight' ? ` This was the PLAYER using Insight on ${names[0]}. On SUCCESS, use Line 3 to reveal one true, specific thing ${names[0]} is feeling or hiding beneath their words — a lie, a fear, a hidden loyalty. On FAILURE, do not write Line 3 at all — revert to the normal two-line format; ${names[0]}'s true feelings stay hidden.` : ''}`
      : '';

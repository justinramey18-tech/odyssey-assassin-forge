...
export function useAIDM({ characterContext, customGuidesContent, worldStatePrompt, dmPersonaPrompt, responseModePrompt, onMessageComplete, onQuestExtracted, activeGuideIds, onCampaignSwitch, selectedModel, sessionStorageKey, summarizeStorageKey }: UseAIDMOptions) {
  ...
  const voiceNPC = useCallback(async (npcNames: string | string[], playerMessage: string) => {
    ...
    const npcVoicingPrompt = names.length === 1
      ? `## NPC VOICING MODE — SUBORDINATE TO HOST OOC AND GM GUIDES\nHost OOC directives are the highest authority. GM Guides are the second-highest authority. This NPC voicing mode is ONLY a formatting constraint layer and must NEVER override Host OOC directives or GM Guides. If either of those requires fewer lines, shorter dialogue, stricter pacing, or different formatting, obey them exactly.\n\nDefault NPC voicing behavior when Host OOC directives and GM Guides do not say otherwise:\n- No novelizations\n- No scene-setting\n- No atmospheric prose\n- No extra paragraphs\n\nYou are voicing ${names[0]} ONLY. Default response shape:\nLine 1: **${names[0]}:** [1–2 sentences of in-character dialogue — MAX 40 words]\nLine 2: *[One physical beat — italicized, present tense, 10 words or fewer]*\n\nIf Host OOC directives or GM Guides impose a stricter limit, follow the stricter rule. STOP as soon as you have satisfied the highest-priority rule. Do not add narration, scene description, other characters' reactions, environmental details, or mechanical tags unless the higher-priority instructions explicitly require them.\n\nExamples of valid default responses:\n\n**${names[0]}:** "I don't owe you an explanation." \n*Turns away, shoulders rigid.*\n\n**${names[0]}:** "You're late. Again." A pause. "Don't let it happen a third time."\n*Doesn't look up from the map.*\n\nStay true to how ${names[0]} has been portrayed so far.`
      : `## NPC VOICING MODE — SUBORDINATE TO HOST OOC AND GM GUIDES\nHost OOC directives are the highest authority. GM Guides are the second-highest authority. This NPC voicing mode is ONLY a formatting constraint layer and must NEVER override Host OOC directives or GM Guides. If either of those requires fewer lines, shorter dialogue, stricter pacing, or different formatting, obey them exactly.\n\nDefault behavior when Host OOC directives and GM Guides do not say otherwise:\n- No novelizations\n- No walls of text\n- Keep the exchange tight and player-facing\n\nWrite a SHORT GROUP CONVERSATION SCENE between ${names.join(' and ')} in response to the player's message. Default rules:\n1. NATURAL TURN ORDER — Let personality decide who speaks first. A bold or reactive NPC jumps in immediately; a cautious one waits and responds to what was already said.\n2. NPCs REACT TO EACH OTHER — Each NPC should acknowledge or respond to what the other NPC(s) just said, not just independently answer the player.\n3. BODY LANGUAGE BEATS — Before or after each line of dialogue, add a brief italicized physical beat in present tense, 10 words or fewer.\n4. 2–3 EXCHANGES TOTAL — Allow a short back-and-forth between the NPCs.\n5. END WITH THE PLAYER — Close on a beat that invites the player back in.\n6. FORMAT — Use **NPC Name:** for every line of dialogue. Italicize all physical beats.\n7. NO MECHANICAL INFO — No dice, DCs, stats, or game-system language in the dialogue.\n\nIf Host OOC directives or GM Guides impose a stricter limit, follow the stricter rule and stop as soon as that rule is satisfied.\n\nStay true to how each NPC has been portrayed so far.`;

...
- SYNC FOOTER (HIGHEST PRIORITY, LEDGER FIELDS ONLY): if the message contains a block delimited by ---SYNC--- and ---END SYNC---, that block is AUTHORITATIVE for the LEDGER FIELDS ONLY. The ledger fields are: hp_changes, hp_absolute, companion_hp_changes, companion_hp_absolute, xp_gained, gold_changes, conditions_added, conditions_removed, items_acquired, items_consumed, rest_occurred. For those fields only, ignore every number in the narrative prose above the footer: the prose is flavour, the footer is the ledger, and if they disagree the footer wins.
- THE FOOTER DOES NOT GOVERN STORY FIELDS. quest_progress, quests_offered, world_state_changes, map_entities and map_entities_removed are ALWAYS read from the narrative prose, whether or not a footer is present. A footer that lists only XP and GOLD does NOT mean no quest advanced. Read the prose for those fields every single time.
- Footer line mapping, exactly:
  ...
  "QUEST: <key>" followed by one or more "OBJECTIVE: <goal text>" lines -> a quest_progress entry for that key with each goal in stages_completed.
  "QUEST COMPLETE: <key or title>" -> a quest_progress entry for that quest with status "completed".
  "QUEST FAILED: <key or title>" -> a quest_progress entry for that quest with status "failed".
  These quest footer lines are OPTIONAL and additive. Their absence never means a quest stood still - always check the prose as well.
  "COMPANION HP: -4" -> companion_hp_changes. "COMPANION HP TOTAL: 12/20" -> companion_hp_absolute.
- A LEDGER line that is absent from the footer means NO CHANGE to that ledger field. Do not infer it from the prose. If the footer omits XP, xp_gained is null even if the prose mentions experience. This applies ONLY to the ledger fields listed above. It NEVER applies to quest_progress, quests_offered or world_state_changes, which are read from the prose regardless.
...
- QUEST PROGRESS: read the NARRATIVE PROSE for this every time, footer or no footer. Mark a stage completed when the prose shows it actually happened, including when the goal is woven into an ordinary sentence rather than presented as a checklist. Match on MEANING, not exact string - the narration and the stored goal will rarely be worded identically. For example, if a goal reads "Pick up the crate from Vessa's stall, floor three" and the narration says "Xeyle picks up the crate from Vessa's stall, floor three" or even "Vessa points at a crate and Xeyle hoists it onto her shoulder", that goal IS completed. Put the goal text COPIED FROM THE ACTIVE QUESTS LIST into stages_completed, not the narration's own wording, so the client can match it. An explicit line anywhere in the message reading "OBJECTIVE COMPLETE: <text>", "QUEST COMPLETE: <name>" or "QUEST FAILED: <name>" is a direct signal and MUST be honoured. Set status "completed" only when the whole job is definitively done and "failed" only when it is irreversibly lost; otherwise use null. Use only keys that appear in the ACTIVE QUESTS list.

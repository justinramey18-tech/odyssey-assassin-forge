...
function pemToArrayBuffer(pem: string): Uint8Array {
  // Handle raw base64url (VAPID private key format — 32 bytes)
  if (!pem.includes('-----')) {
    return base64UrlDecode(pem);
  }
...
async function importVapidKeys(): Promise<{ publicKey: Uint8Array; privateKey: CryptoKey }> {
  ...
  // Import raw 32-byte private key as JWK for P-256
  ...
  const payload = base64UrlEncode(
    new TextEncoder().encode(
      JSON.stringify({
        aud: audience,
        exp: now + 12 * 3600,
        sub: 'mailto:push@infinitypoolassassin.lovable.app',
      }),
    ),
  );
...
async function sendWebPush(
  ...
  try {
    // Generate ephemeral ECDH keys for encryption
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. Get push subscriptions for those members
    ...
    // 4. Send Web Push to each subscription
    ...
    const results = await Promise.allSettled(
      subscriptions.map((sub) =>
        sendWebPush(sub, payloadStr, vapid.publicKey, vapid.privateKey),
      ),
    );

    // 5. Clean up expired subscriptions (410/404)

...
A bar at the bottom plays the whole message start to finish using your custom order, so you can check the draft in one go. A top action voices everything still silent in one pass.
...
- **Set its play speed** (0.5x to 2x). Playback only — it does not re-voice anything and costs nothing.

...
- **Split it** into two pieces at a point you choose, or **merge** it with the piece below, so a long paragraph can get two voices or two short lines can share one. Splitting is unlimited: any piece produced by a split can itself be split again, down to a single word. A one-word piece is the smallest possible and can't be split further.
